...
import json
import logging
from query_orchestrator import main as query_orchestrator_main  # Import the query orchestrator
...
def evaluate_number_of_trials(trials, config):
    ...
    if num_trials == 0:
        logging.info("No trials found. Triggering query orchestrator to increase radius.")
        print("No Trials")
        query_orchestrator_main()  # Run the query orchestrator to increase the radius and try again
        return "no_trials"
...
def main():
    ...
    if evaluation_result == "no_trials":
        logging.info("No trials found after radius expansion. Exiting process.")
    elif evaluation_result == 'Few trials':
